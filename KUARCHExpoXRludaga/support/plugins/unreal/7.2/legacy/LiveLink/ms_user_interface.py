...
megascans_folder = (os.path.join(ms_return_path(), 'megascans', 'ui'))


class MsMainClass(object):
...
Pause_Style = ("""
    QPushButton { font-size: 16px; border: 0px; font-family: Source Sans Pro;background-color: #2B2B2B; border-radius: 1px; color : #E6E6E6; }
    QPushButton:hover { font-size: 16px; border: 0px; font-family: Source Sans Pro;background-color: #2daae1; border-radius: 1px; color : #E6E6E6; }
        """)
...
class QBaseButton(QPushButton):
    ...
    def __init__(self, request):
        ...
        self.setIcon( QIcon(QPixmap(os.path.join(megascans_folder, self.request + '_icon.png'))))
    ...
    def enterEvent(self, event):
        self.setIcon( QIcon(QPixmap(os.path.join(megascans_folder, self.request + '_hover_icon.png'))))
        self.setIconSize(QSize(22, 22))
...
class QCustomCheckBox(QWidget):
    ...
    def __init__(self, request, checkstate, parent):
        ...
        self.path_ = (os.path.join(megascans_folder, 'Checked_Black.png'))
        self.request = request
        self.parent = parent
        ...
        self.text_.setStyleSheet("QLabel {background-color: transparent;font-size: 16px;"
                                 " font-family: Source Sans Pro; border: 0px; color: #ffffff;}")
        # self.text_.setFixedHeight(30)
        self.mainlayout.addWidget(self.text_)

        if self.Checked:
            self.icon_.setPixmap(QPixmap(self.path_))
            self.icon_.setStyleSheet("QLabel {background-color: transparent; border-radius: 2px;"
                                     "border: 2px solid #6F6F6F; color: #a2a2a2;}")

        else:
            self.icon_.setPixmap(QPixmap(None))
            self.icon_.setStyleSheet("QLabel {background-color: #2B98F0; border-radius: 2px;"
                                     "border: 0px; color: #a2a2a2;}")
    ...
    def enterEvent(self, event):
        if self.Checked:
            self.icon_.setStyleSheet("QLabel {background-color: #3183c5; border-radius: 2px;"
                                     "border: 0px; color: #a2a2a2;}")
        else:
            self.icon_.setStyleSheet("QLabel {background-color: transparent; border-radius: 2px;"
                                     "border: 2px solid #2B98F0; color: #a2a2a2;}")
...
class MsAboutWin(QDialog):

    def __init__(self, parent=None):
        super(MsAboutWin,self).__init__(parent)
        ...
        style_ = ("QLabel {background-color: transparent; font-size: 11pt; font-family: Ubuntu;"
                  " Source Sans Pro: 0px;color: #ffffff;}")

        style_2 = ("QLabel {background-color: transparent; font-size: 11pt; font-family: Ubuntu;"
                   " Source Sans Pro: 0px;color: #a4a4a4;}")
...
class megascans_livelink_ui(QDialog):
    ...
    def __init__(self):
        ...
        self.setWindowIcon(QIcon(os.path.join(megascans_folder, 'MS_Logo.png')))
        self.setWindowFlags(Qt.FramelessWindowHint)
        ...
        self.logo_.setIcon( QIcon(QPixmap(os.path.join(megascans_folder, 'MS_Logo.png'))))
        ...
        self.help_ = QBaseButton("help")
        # self.help_.setToolTip("Minimize the Megascans LiveLink window")
        ...
        for item in [item[0] for item in MsMainClass.materials]:
        ...
        self.mtl_name = QMaterialDisplay(MsMainClass.materials[0][1], self)
    ...
    def open_quick_help(self):
        webbrowser.open('https://www.youtube.com/watch?v=oBP_NZtCwvg&list=PLeVoIk2cZ8giB4VyyGcur_n0fptS63K_S')
    ...
    def Change_Material_ui(self):
        ...
        try:
            ...
            self.mtl_name.setText(MsMainClass.materials[indx_][1])
        except:
            pass
    ...
    def sync_mtl_browser(self):
        ...
        try:
            cur_ = MsMainClass.materials[self.obj_type.currentIndex()]
    ...
    def update_scale(self):
        ...
        try:
            ...
            scaled_value = float(value_)/10

            actors_ = ue.editor_get_selected_actors()
            if len(actors_) >= 1:
                for actor_ in actors_:
                    if actor_.is_a(StaticMeshActor):
                        actor_.set_actor_scale(scaled_value, scaled_value, scaled_value)
                ...
                try:
                    if sel.is_a(StaticMeshActor):
                        ...
                        if len(mtl) >= 1:
                            ...
                            mtl.set_material_scalar_parameter('Tiling', scaled_value)
    ...
    def set_new_mat(self):
        ...
        indx_ = self.obj_type.currentIndex()
        input_ = (MsMainClass.materials[indx_][0])

        with open(ms_return_path() + '/megascans/settings.json') as f:
            mat_settings = json.loads(f.read())
        ...
        if package_ is not None and package_.is_a(Material):

            obj_mat = [item for item in mat_settings['CustomMaterial'] if item[0].lower() == input_.lower()][0]

            mat_indx = mat_settings['CustomMaterial'].index(obj_mat)

            if package_name is not None and len(obj_mat) >= 2:
                ...
                if len(obj_mat) == 3:
                    ...
                    mat_settings['CustomMaterial'][mat_indx] = obj_mat

            Export_ = json.dumps((dict(mat_settings)), sort_keys=True, ensure_ascii=False, indent=2)
            ...
            MsMainClass.materials[indx_][1] = obj_mat[1]
            self.mtl_name.setText(MsMainClass.materials[indx_][1])
